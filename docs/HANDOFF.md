...
- canonical repository commands include lint, typecheck, unit test, E2E, and build.
...
1. **Feature 02 sequencing gate.** Prior AgentCal Product Truth identified the Tier-2 export/share artifact as the remaining M1 dependency. The 2026-09-11 product split does not cancel it. Product Owner priority must explicitly determine whether Feature 02 is completed before BUY or deliberately reprioritized.
2. **BUY engine not implemented.** No `lib/engine/buy.ts` exists on current `main`.
3. **MOVE engine not implemented.** MOVE should follow verified BUY and compose verified SELL + BUY results rather than reimplementing them.
4. **No active BUY Technical Plan.** Do not start implementation until the sequencing gate is resolved and a bounded plan is created.
5. **Cross-product integration is deferred.** AgentCal ↔ AgentConsult should later use a small explicit versioned scenario/result contract only when both products have stable shapes and a bounded integration task is approved.
6. **Real-client disclaimer review remains relevant** where prior Product Truth required managing-broker / BC counsel review before real-client use; the product split does not silently remove that requirement.

## Next

**Next bounded action: resolve AgentCal feature sequencing at the Product Owner gate — outstanding Feature 02 export/share versus the next BUY calculation-engine milestone.**
