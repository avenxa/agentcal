export const TIER1_DISCLAIMER =
  "Estimate only — not a quote, approval, or professional advice.";

export const TAX_EXCLUSION_NOTE =
  "Capital gains tax and BC home flipping tax are not calculated. Use an adviser-supplied amount only as a manual adjustment.";

export const COMMISSION_PRESET_LABEL = "Typical BC preset";
export const COMMISSION_PRESET_FORMULA = "7% first $100k + 2.5% balance";
export const COMMISSION_NEGOTIABLE_NOTICE =
  "Commission is negotiable. Confirm the amount in the listing agreement.";

export const SALE_PRICE_ERROR = "Enter an amount greater than $0.";
export const MORTGAGE_WARNING =
  "Mortgage payout exceeds the expected selling price.";
export const NEGATIVE_RESULT_NOTE =
  "Estimated costs and mortgage exceed the selling price.";
export const EMPTY_RESULT_LABEL = "Enter selling price";
export const UNAVAILABLE_AMOUNT = "—";
